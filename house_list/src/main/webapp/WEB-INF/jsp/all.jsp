...
<body>
	...
	<form action="/house_list/Search" method="post">
		...
		<!-- <input type="date" id="moving_date_from" name="moving_date_from" value="">
    ～<input type="date" id="moving_date_to" name="moving_date_to" value=""><br> -->
		<label for="housingType"></label> 
		<select id="housingType"name="housingType">
			<option value="">物件タイプを選択</option>
			<c:forEach items="${uniqueHousingTypes}" var="housingType">
				<option id="housing_type" name="housing_type" value="${housingType}">${housingType}</option>
			</c:forEach>
		</select> 
